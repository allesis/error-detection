...
class Model:
    @type_enforced.Enforcer(enabled=True, strict=True, clean_traceback=True)
    def train(self, source: numpy.ndarray, target: numpy.ndarray = numpy.empty(0)):
    ...
    def evaluate(
        ...
        """
        return self._model.score(source, target)
    ...
    def predict(
        ...
        The model will train itself it is has not been prior to this method executing.
        """
        return list(map(lambda x: 0.0 + x, self._model.fit_predict(source, target)))
